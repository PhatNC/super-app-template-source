...
RCT_EXPORT_METHOD(sendMessage:(NSString *)bundleName msg:(NSDictionary *)msg callback:(RCTResponseSenderBlock)callback)
{
    ...
    if (emitter) {
        ...
        NSString *str = [NSString stringWithFormat:@"[sendMessage] Cannot find this bundle name %@", bundleName];
...
{
    RCTPromiseResolveBlock promise = [promises objectForKey:requestId];
    NSMutableDictionary *result = [NSMutableDictionary new];
    if (promise) {
        promise(response);
        ...
        NSString *str = [NSString stringWithFormat:@"[replyResponse] Cannot find promise with id %@", requestId];
        [result setObject:str forKey:@"msg"];
    }
    callback(@[result]);
}
